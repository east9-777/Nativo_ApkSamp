package com.nativo.rpg.game.ui.widgets.adapter;
...
public class DialogAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public ViewHolder(View view) {
            super(view);
            this.mView = view;
            this.mRowBg = view.findViewById(R.id.item_bg);
            this.mFields[0] = view.findViewById(R.id.item_field1);
            this.mFields[1] = view.findViewById(R.id.item_field2);
            this.mFields[2] = view.findViewById(R.id.item_field3);
            this.mFields[3] = view.findViewById(R.id.item_field4);
        }
    ...
    public DialogAdapter(ArrayList<String> arrayList, Activity activity, boolean z) {
    ...
    public void updateSizes(TextView textView) {
        ...
        for (int i = 0; i < this.mRowsList.size(); i++) {
            ...
            if (!this.mTablist) {
                float textWidth = ((float) Utils.getTextWidth(Utils.getColoredString(str).toString(), textView.getPaint())) + Utils.scale(this.mContext, 32.0f);
                int[] iArr2 = this.mTabSizes;
                ...
                for (int i2 = 0; i2 < length; i2++) {
                    float textWidth2 = ((float) Utils.getTextWidth(Utils.getColoredString(split[i2]).toString(), textView.getPaint())) + Utils.scale(this.mContext, 32.0f);
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
        View inflate = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.layout_dialog_item, viewGroup, false);
    ...
    public void onBindViewHolder(ViewHolder viewHolder) {
        ...
        if (!this.mTablist) {
            ...
            for (int i3 = 0; i3 < 4; i3++) {
                if (length > i3) {
                    viewHolder.mFields[i3].setText(Utils.getColoredString(split[i3]));
                }
            }
        }
        ...
        viewHolder.getMView().setOnClickListener(view -> {
