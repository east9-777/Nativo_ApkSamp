...
void CGangZonePool::New(uint16_t wZoneID, float fMinX, float fMinY, float fMaxX, float fMaxY,
		uint32_t dwColor)
{
...
void CGangZonePool::Draw(bool bFullMap)
{
	...
	for (uint16_t wZone = 0; wZone < MAX_GANG_ZONES; wZone++) {
		if (m_bSlotState[wZone]) {
			pGame->DrawGangZone(m_pGangZone[wZone]->fPos,alt ? m_pGangZone[wZone]->dwAltColor : m_pGangZone[wZone]->dwColor, bFullMap);
		}
	}
}