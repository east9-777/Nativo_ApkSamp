...
inline void LogVoice(const char* fmt, ...)
{
	char buffer[0xFF];
	static FILE* flLog = nullptr;

	if (flLog == nullptr && Client::gameDir() != nullptr) {
		sprintf(buffer, "%sSAMP/%s", Client::gameDir(), SV::kLogFileName);
	...
	__android_log_write(ANDROID_LOG_INFO, "Alyn_SAMPMOBILE", buffer);
