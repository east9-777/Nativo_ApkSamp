...
StreamAtPoint::StreamAtPoint(const uint32_t color, std::string name,
		const float distance, const sa::CVector& position) noexcept
		: LocalStream(StreamType::LocalStreamAtPoint, color, std::move(name), distance), position(position) { }

void StreamAtPoint::SetPosition(const sa::CVector& position) noexcept
{
