...
bool Record::Init(const uint32_t bitrate) noexcept
{
	...
	SVMemory::ScopeExit deviceListsResetScope{[] {
	...
	SVMemory::ScopeExit encoderResetScope{[] { opus_encoder_destroy(Record::encoder); }};
	...
	SVMemory::ScopeExit recordResetScope{[] { BASS_RecordFree(); }};
	...
	SVMemory::ScopeExit channelResetScope{[] { BASS_ChannelStop(Record::recordChannel); }};
...
uint32_t Record::GetFrame(uint8_t* const bufferPtr, const uint32_t bufferSize) noexcept
{
	...
	if (BASS_ChannelGetData(Record::recordChannel, Record::encBuffer.data(),
			SV::kFrameSizeInBytes) != SV::kFrameSizeInBytes) {
		return NULL;
	}
